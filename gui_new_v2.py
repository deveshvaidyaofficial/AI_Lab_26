...
class SearchAlgorithm:
    ...
    The goal is OPTIONAL. Only a source/start node is required:
        - If a goal is given, the search stops as soon as the goal is
          expanded and reconstructs the start->goal path (original
          behaviour).
        - If no goal is given (goal is None), the search instead runs
          a full traversal, visiting every node reachable from the
          source, and only stops once the frontier is empty.
    ...
    Public results, matching the behaviour of the original BFS/DFS:
        ...
        found: True if the goal was reached (always False when no goal
            was specified, since there is nothing to "find")
    ...
    def __init__(self, graph, start, goal=None):
        self.graph = graph
        self.start = start
        self.goal = goal              # may be None: source-only traversal
    ...
    def step(self):
        ...
        When no goal was specified this simply keeps expanding nodes
        ...
        if self.goal is not None and node_id == self.goal:
            self.found = True
            self.finished = True
            self._build_path()
            return True
...
class GraphEditor:
    ...
    def _build_toolbar(self):
        ...
        self.goal_input = self.frame.add_input(
            "Set Goal Node # (optional)", self.handle_goal_input, 60
        )
    ...
    def handle_goal_input(self, text):
        ...
        if text.strip() == "":
            self.goal_node_id = None
            self.status_message = "Goal cleared. BFS/DFS will run as a full traversal from the source."
            return
        node_id = self._parse_valid_node_id(text)
        if node_id is None:
    ...
    def _start_search(self, algorithm_cls, name):
        ...
        if self.start_node_id is None:
            self.status_message = "Set a start (source) node first."
            return

        # The goal is optional: if none was set, this becomes a full
        # traversal from the source that visits every reachable node.
        self.graph.reset_marks()
        self.algorithm = algorithm_cls(self.graph, self.start_node_id, self.goal_node_id)
        self.algorithm_name = name
        self.animation_running = True
        if self.goal_node_id is None:
            self.status_message = "Running {0} from source {1} (no goal set)...".format(
                name, self.start_node_id
            )
        else:
    ...
    def _finalize_search(self):
        if self.algorithm is None:
            return
        if self.goal_node_id is None:
            # Source-only traversal: report every node that was reached.
            self.status_message = "{0} traversal from source {1} complete. Visited: {2}".format(
                self.algorithm_name,
                self.start_node_id,
                self._format_path(self.algorithm.visited_order),
            )
            self._export_traversal_result()
        elif self.algorithm.found:
    ...
    def _draw_status_bar(self, canvas):
        ...
        mode_text = "SEARCH MODE" if self.locked else "EDIT MODE - " + self.mode
